...
export class ResumeComparer {
    compare(profileText, resumeText) {
    ...
    calculateAlignmentScore(profileSections, resumeSections) {
        let score = 0;
        let totalSections = 0;
    ...
    findMissingFromProfile(profileSections, resumeSections) {
        const missing = {};
        for (const section of ['skills', 'experience', 'education', 'certifications', 'projects']) {
            const profileItems = new Set((profileSections[section] || []).map((item) => item.toLowerCase()));
            const resumeItems = resumeSections[section] || [];
            missing[section] = resumeItems.filter((item) => !profileItems.has(item.toLowerCase()));
        }
        return missing;
    }
    findMissingFromResume(profileSections, resumeSections) {
        const missing = {};
        for (const section of ['skills', 'experience', 'education', 'certifications', 'projects']) {
            const resumeItems = new Set((resumeSections[section] || []).map((item) => item.toLowerCase()));
            const profileItems = profileSections[section] || [];
            missing[section] = profileItems.filter((item) => !resumeItems.has(item.toLowerCase()));
        }
        return missing;
    }
    findConsistencyIssues(profileSections, resumeSections) {
        const issues = [];
    ...
    hasCommonEducation(profileEducation, resumeEducation) {
    ...
    generateAlignmentRecommendations(profileSections, resumeSections) {
...
//# sourceMappingURL=resumeComparer.js.map