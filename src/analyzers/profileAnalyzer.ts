...
export class ProfileAnalyzer {
  ...
  private extractHeadline(lines: string[]): { content: string; quality: string } {
  ...
  private extractSummary(lines: string[]): { content: string; quality: string } {
  ...
  private extractExperience(lines: string[]): { content: string[]; quality: string } {
  ...
  private extractSkills(lines: string[]): { content: string[]; quality: string } {
  ...
  private extractEducation(lines: string[]): { content: string[]; quality: string } {
  ...
  private extractCertifications(lines: string[]): { content: string[]; quality: string } {
  ...
  private extractProjects(lines: string[]): { content: string[]; quality: string } {
  ...
  private assessHeadlineQuality(headline: string): string {
    if (!headline || headline === 'No clear headline found') return 'Poor - Missing headline';
    if (headline.length < 30) return 'Fair - Too short';
    if (headline.length > 120) return 'Fair - Too long';
    if (headline.includes('seeking') || headline.includes('looking')) return 'Poor - Sounds passive';
    return 'Good - Clear and professional';
  }

  private assessSummaryQuality(summary: string): string {
    if (!summary || summary === 'No summary found') return 'Poor - Missing summary';
    if (summary.length < 100) return 'Fair - Too brief';
    if (summary.length > 2000) return 'Fair - Too long';
    return 'Good - Comprehensive summary';
  }

  private assessExperienceQuality(experiences: string[]): string {
    if (experiences.length === 0) return 'Poor - No experience listed';
    if (experiences.length < 2) return 'Fair - Limited experience shown';
    return 'Good - Multiple experiences listed';
  }

  private assessSkillsQuality(skills: string[]): string {
    if (skills.length === 0) return 'Poor - No skills listed';
    if (skills.length < 5) return 'Fair - Limited skills shown';
    if (skills.length > 50) return 'Fair - Too many skills (diluted focus)';
    return 'Good - Balanced skills list';
  }

  private assessEducationQuality(education: string[]): string {
    if (education.length === 0) return 'Fair - No education listed';
    return 'Good - Education information present';
  }

  private assessCertificationsQuality(certifications: string[]): string {
    if (certifications.length === 0) return 'Fair - No certifications listed';
    return 'Good - Certifications present';
  }

  private assessProjectsQuality(projects: string[]): string {
    if (projects.length === 0) return 'Fair - No projects mentioned';
  ...
  private calculateCompleteness(sections: any): number {
    ...
    if (sections.headline.content !== 'No clear headline found') score += weights.headline;
    if (sections.summary.content !== 'No summary found') score += weights.summary;
    if (sections.experience.content.length > 0) score += weights.experience;
    if (sections.skills.content.length > 0) score += weights.skills;
    if (sections.education.content.length > 0) score += weights.education;

    return score;
  }

  private calculateStrength(sections: any, keywords: any): number {
    let strength = 0;
    ...
    // Content quality
    const qualityScores = Object.values(sections as Record<string, any>).map((section: { quality?: string }) => {
      if (section.quality?.includes('Good')) return 10;
      if (section.quality?.includes('Fair')) return 5;
      return 0;
    });
    strength += qualityScores.reduce((a: number, b: number) => a + b, 0) * 0.5;

    return Math.min(Math.round(strength), 100);
  }

  private generateRecommendations(sections: any, completeness: number): string[] {
  ...
  private generateOverallAssessment(strength: number, completeness: number): string {
